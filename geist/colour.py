...
def rgb_to_hsv(image):
    ...
    hue_r = (((g.astype(numpy.float32) - b)/hue_div)*43).astype(numpy.uint8)
    hue_r[r != max_rgb] = 0
    hue_g = ((((b.astype(numpy.float32) - r)/hue_div)*43)+85).astype(numpy.uint8)
    hue_g[g != max_rgb] = 0
    hue_b = ((((r.astype(numpy.float32) - g)/hue_div)*43)+171).astype(numpy.uint8)
    hue_b[b != max_rgb] = 0
